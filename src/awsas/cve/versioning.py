        ...
        for node in nodes:
            ...
            if isinstance(matches, list):
                for m in matches:
                    ...
                    # cpe URI: cpe23Uri (old) or criteria (new)
...
def assess_version_against_cve(
    detected_version: str,
    cve_item: Dict[str, Any],
    allowed_cpe_prefixes: Optional[List[str]] = None,
) -> str:
    ...
    if allowed_cpe_prefixes:
        filtered = []
        for r in ranges:
            cpe_uri = r.get("cpe_uri")
            if isinstance(cpe_uri, str) and any(cpe_uri.startswith(p) for p in allowed_cpe_prefixes):
                filtered.append(r)
        ranges = filtered
    ...
    for r in ranges:
        if r.get("type") == "exact":
            ...
            if not v:
                continue
            if _cmp_versions(detected_version, v) == 0:
                return "vulnerable"

        elif r.get("type") == "range":
            start = r.get("start")
            end = r.get("end")
            inc_start = bool(r.get("start_including", False))
            inc_end = bool(r.get("end_including", False))
            ...
            if start:
                ...
                ok_lower = (cmp_low >= 0) if inc_start else (cmp_low > 0)

            if end:
                cmp_high = _cmp_versions(detected_version, end)
                ok_upper = (cmp_high <= 0) if inc_end else (cmp_high < 0)
